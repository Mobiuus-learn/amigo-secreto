...
export const doMatchs = async (id: number): Promise<boolean> => {
	...
	if (eventItem) {
		...
		if (peopleList) {
			...
			while (keepTrying && attempts < maxAttempts) {
				...
				for (let i in peopleList) {
					...
					if (
						...
						(sortableFiltered.length === 1 && peopleList[i].id === sortableFiltered[0])
					) {
						keepTrying = true;
					} else {
			...
			// 		await people.updatePeople({ id: sortedList[i].id, id_event: id }, { matched: "" }); // TODO : criar encripted match
			// 	}
			//   return true
			// }
		}
